...
import { PlusCircle, Edit, Trash2, Search, ShoppingCart, Calendar as CalendarIcon, MinusCircle } from 'lucide-react';
...
export function PurchaseNotesClient() {
  ...
  return (
    <div>
            ...
            <TableBody>
              {isLoading ? Array.from({ length: 3 }).map((_, i) => (
                ...
                <TableRow key={note.id}>
                  ...
                  <TableCell>{note.supplierName}</TableCell><TableCell className="text-right">{formatCurrency(note.grandTotal, note.currency)}</TableCell>
      ...
      <PurchaseNoteDialog isOpen={isDialogOpen} onClose={() => setIsDialogOpen(false)} onSave={handleSaveNote} note={editingNote} />
    </div>
  );
}
