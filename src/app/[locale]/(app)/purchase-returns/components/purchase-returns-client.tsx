...
import type { PurchaseReturnNote, DocumentItem, Product, TaxRate } from '@/lib/types';
...
import { PlusCircle, Edit, Trash2, Search, Undo2, Calendar as CalendarIcon, MinusCircle, Printer } from 'lucide-react';
...
import { initialPurchaseReturnNotes, mockProducts, mockTaxRates } from '@/lib/mock-data';
...
export function PurchaseReturnsClient() {
  const [notes, setNotes] = useState<PurchaseReturnNote[]>([]);
  const [isLoading, setIsLoading] = useState(true);
  const [isDialogOpen, setIsDialogOpen] = useState(false);
  const [editingNote, setEditingNote] = useState<PurchaseReturnNote | null>(null);
  const [searchTerm, setSearchTerm] = useState('');
  const currencySymbol = '$'; // Simulate fetching setting

  useEffect(() => {
    setTimeout(() => {
      setNotes(initialPurchaseReturnNotes);
      setIsLoading(false);
    }, 1000);
  }, []);
  ...
  const handleEditNote = (note: PurchaseReturnNote) => {
    setEditingNote(note);
    setIsDialogOpen(true);
  };
  const handleDeleteNote = (id: string) => setNotes(notes.filter(n => n.id !== id));
  
  const handleSaveNote = (noteData: Omit<PurchaseReturnNote, 'id' | 'currency'> & { id?: string }) => {
    if (editingNote && noteData.id) {
      setNotes(notes.map(n => (n.id === noteData.id ? { ...n, ...noteData, currency: 'USD' } : n)));
    } else {
      setNotes([...notes, { ...noteData, id: `prn${Date.now()}`, currency: 'USD' } as PurchaseReturnNote]);
    }
  ...
  const filteredNotes = useMemo(() => notes.filter(note =>
    ...
    note.originalPurchaseNoteNumber.toLowerCase().includes(searchTerm.toLowerCase())
  ), [notes, searchTerm]);
    ...
    <div>
      <div className="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4">
        <div className="flex items-center gap-2">
            ...
            <h1 className="text-3xl font-bold text-primary">Purchase Returns</h1>
        </div>
      ...
      <div className="mb-6 p-4 border rounded-lg shadow-sm bg-card">
        <div className="relative">
          ...
          <Input 
            type="text" 
            placeholder="Search by return note #, supplier, or original note #..." 
            value={searchTerm}
            ...
            <TableBody>
              {isLoading ? Array.from({ length: 1 }).map((_, i) => (
                ...
                <TableRow key={note.id}>
                  ...
                  <TableCell>{note.originalPurchaseNoteNumber}</TableCell>
                  <TableCell className="text-right">{formatCurrency(note.grandTotal, currencySymbol)}</TableCell>
                ...
                <TableRow><TableCell colSpan={6} className="text-center h-24">No purchase returns found.</TableCell></TableRow>
              )}
            </TableBody>
          </Table>
        </CardContent>
      </Card>
      <PurchaseReturnNoteDialog isOpen={isDialogOpen} onClose={() => setIsDialogOpen(false)} onSave={handleSaveNote} note={editingNote} currencySymbol={currencySymbol} />
    </div>
  );
}

// Dialog Component
interface PurchaseReturnNoteDialogProps {
  isOpen: boolean;
  onClose: () => void;
  onSave: (noteData: Omit<PurchaseReturnNote, 'id' | 'currency'> & { id?: string }) => void; 
  note: PurchaseReturnNote | null;
...
function PurchaseReturnNoteDialog({ isOpen, onClose, onSave, note, currencySymbol }: PurchaseReturnNoteDialogProps) {
  const [formData, setFormData] = useState<Partial<Omit<PurchaseReturnNote, 'currency'>>>({});
  ...
  useEffect(() => {
    if (isOpen) {
      if (note) {
        ...
        setFormData({
          noteNumber: `PRN${Date.now().toString().slice(-4)}`,
          date: new Date(),
          supplierName: '',
          originalPurchaseNoteNumber: '',
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!formData.noteNumber || !formData.date || !formData.supplierName || !formData.originalPurchaseNoteNumber) {
      alert("Return Note #, Date, Supplier Name and Original Note # are required.");
      return;
    }
    ...
    onSave(finalNote as Omit<PurchaseReturnNote, 'id' | 'currency'> & { id?: string });
  };
    ...
    <Dialog open={isOpen} onOpenChange={onClose}><DialogContent className="max-w-5xl shadow-xl">
      ...
      <form onSubmit={handleSubmit}><div className="grid gap-6 py-4">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <div><Label htmlFor="originalPurchaseNoteNumber">Original Note #</Label><Input id="originalPurchaseNoteNumber" name="originalPurchaseNoteNumber" value={formData.originalPurchaseNoteNumber || ''} onChange={handleHeaderChange} required /></div>
        </div>
