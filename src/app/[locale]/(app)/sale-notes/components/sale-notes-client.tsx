...
import type { SaleNote, DocumentItem, Product, TaxRate } from '@/lib/types';
...
export function SaleNotesClient() {
  ...
  const handleEditNote = (note: SaleNote) => {
    setEditingNote(note);
    setIsDialogOpen(true);
  };
  const handleDeleteNote = (id: string) => setNotes(notes.filter(n => n.id !== id));
  
  const handleSaveNote = (noteData: Omit<SaleNote, 'id' | 'currency'> & { id?: string }) => {
    if (editingNote && noteData.id) {
      setNotes(notes.map(n => (n.id === noteData.id ? { ...n, ...noteData, currency: 'USD' } : n)));
    } else {
      setNotes([...notes, { ...noteData, id: `sn${Date.now()}`, currency: 'USD' } as SaleNote]);
    }
    ...
    <div>
      <div className="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4">
        <div className="flex items-center gap-2">
            ...
            <h1 className="text-3xl font-bold text-primary">Sale Notes</h1>
        </div>
        <Button onClick={handleAddNote} className="shadow-md hover:shadow-lg">
          <PlusCircle className="mr-2 h-5 w-5" /> Add Sale Note
        </Button>
      </div>
      <div className="mb-6 p-4 border rounded-lg shadow-sm bg-card">
        <div className="relative">
          ...
          <Input 
            type="text" 
            placeholder="Search by note #, customer, or product..." 
            value={searchTerm}
        ...
        <CardContent className="p-0">
          <Table>
            <TableHeader><TableRow>
              <TableHead>Note #</TableHead><TableHead>Date</TableHead><TableHead>Customer</TableHead>
            ...
            <TableBody>
              {isLoading ? Array.from({ length: 3 }).map((_, i) => (
                ...
                <TableRow><TableCell colSpan={5} className="text-center h-24">No sale notes found.</TableCell></TableRow>
              )}
            </TableBody>
          </Table>
        </CardContent>
...
function SaleNoteDialog({ isOpen, onClose, onSave, note, currencySymbol }: SaleNoteDialogProps) {
  ...
  useEffect(() => {
    if (isOpen) {
      if (note) {
        ...
        setFormData({
          noteNumber: `SN${Date.now().toString().slice(-4)}`,
          date: new Date(),
          customerName: '',
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!formData.noteNumber || !formData.date || !formData.customerName) {
      alert("Note #, Date, and Customer Name are required.");
      return;
    }
