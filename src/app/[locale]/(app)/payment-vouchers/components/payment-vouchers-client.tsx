...
import type { PaymentVoucher, VoucherMode } from '@/lib/types';
...
import { PlusCircle, Edit, Trash2, Search, ArrowBigUpDash, Calendar as CalendarIcon, Printer } from 'lucide-react';
...
export function PaymentVouchersClient() {
  ...
  useEffect(() => {
    // Simulate API call
    setTimeout(() => {
      setVouchers(initialPaymentVouchers);
      setIsLoading(false);
    }, 1000);
  }, []);
  ...
  const handleSaveVoucher = (voucherData: Omit<PaymentVoucher, 'id' | 'currency'> & { id?: string }) => {
    if (editingVoucher && voucherData.id) {
      setVouchers(vouchers.map(v => (v.id === voucherData.id ? { ...v, ...voucherData, currency: 'USD' } : v)));
    } else {
      setVouchers([...vouchers, { ...voucherData, id: String(Date.now()), currency: 'USD' } as PaymentVoucher]);
    }
    ...
    <div>
      <div className="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4">
        <div className="flex items-center gap-2">
            ...
            <h1 className="text-3xl font-bold text-primary">Payment Vouchers</h1>
        </div>
        <Button onClick={handleAddVoucher} variant="default" className="shadow-md hover:shadow-lg">
          <PlusCircle className="mr-2 h-5 w-5" /> Add Payment Voucher
        </Button>
      </div>
...
function VoucherDialog({ isOpen, onClose, onSave, voucher, voucherType }: VoucherDialogProps) {
  const [formData, setFormData] = useState<Partial<Omit<PaymentVoucher, 'currency'>>>({});
    ...
    if (isOpen) {
      if (voucher) {
        ...
        setFormData({
          ...
          date: defaultDate,
          partyName: '',
          paymentMode: voucherModeOptions[0].value,
          amount: 0,
          narration: '',
        });
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    onSave(formData as Omit<PaymentVoucher, 'id' | 'currency'> & { id?: string });
  };
        ...
        <form onSubmit={handleSubmit}>
          <div className="grid gap-4 py-4">
            ...
            <div className="grid grid-cols-4 items-center gap-4">
              ...
              <Select name="paymentMode" value={formData.paymentMode} onValueChange={(value) => handleSelectChange('paymentMode', value)} required>
